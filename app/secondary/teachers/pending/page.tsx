...
import { Loader2, CheckCircle, XCircle, UserPlus, AlertCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function PendingApprovalPage() {
  ...
  useEffect(() => {
    ...
    // 🔥 Check if user has admin rights (Secondary/Advanced)
  ...
  const fetchPendingTeachers = async (authToken: string) => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/teachers/pending`, {
  ...
  const handleApprove = async (teacherId: number) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/teachers/${teacherId}/approve`, {
        method: "PUT",
        headers: {
  ...
  const handleReject = async (teacherId: number) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/teachers/${teacherId}/reject?reason=${encodeURIComponent(rejectionReason)}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-700 via-indigo-700 to-purple-700 p-8 text-white shadow-xl">
          <div className="relative z-10">
            ...
            <h1 className="text-2xl font-bold">Teacher Approval</h1>
          ...
          <CardContent className="p-0">
            {pendingTeachers.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-lg font-medium">No pending teachers</p>
                <p className="text-sm">All teachers have been approved</p>
              </div>
            ) : (
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to reject <strong>{selectedTeacher?.name}</strong>?
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-gray-400">This reason will be shown to the rejected teacher</p>
              </div>
            </div>
            <DialogFooter>
