...
export default function SecondaryPastPapersPage() {
  ...
  // 🔥 DEFAULT: secondary kwa secondary users
  ...
  // 🔥 BADILISHA: FETCH PAST PAPERS - "all" inamaanisha level ya user
  const fetchPastPapers = async (authToken: string) => {
    try {
      ...
      // 🔥 IMPORTANT: "all" inamaanisha filter kwa user's school level
  ...
  // 🔥 School Level Options - Dynamic kulingana na user
  const getSchoolLevelOptions = () => {
    const userLevel = userSchoolLevel || "secondary"
    
    // 🔥 User anaona tu level yake au All Schools (ambayo ni level yake pia)
    const options = [
      { 
        value: "all", 
        label: `🏫 All ${userLevel.charAt(0).toUpperCase() + userLevel.slice(1)} Schools` 
      },
      { 
        value: userLevel, 
        label: userLevel === "primary" ? "🏫 Primary" : 
                userLevel === "secondary" ? "📚 Secondary" : 
                "🎓 Advanced" 
      }
    ]
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (formData.file.size > maxSize) {
      setError(`File too large! Maximum size is 10MB. Your file is ${(formData.file.size / (1024 * 1024)).toFixed(2)}MB`)
      return
    }
  ...
  const canUpload = ["Teacher", "Academic", "Headmaster", "Headmistress", "Second Master", "Second Mistress", "Mwalimu", "Mtaaluma", "Mwalimu Mkuu", "Mwalimu Mkuu Msaidizi"].includes(userRole)
    ...
    return (
      <MainLayout>
        <div className="flex flex-col items-center justify-center h-96">
          ...
          <p className="text-gray-500 mt-4 animate-pulse">Loading past papers...</p>
        </div>
      </MainLayout>
    )
  }
  ...
  return (
    <MainLayout>
      <div className="space-y-6 p-6 animate-fadeIn">
        {/* Header Section */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-700 via-orange-700 to-red-700 p-8 text-white shadow-xl">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-3xl font-bold mb-2">📚 Past Papers Library</h1>
            <p className="text-amber-100 max-w-2xl">
              Browse and download past examination papers from {userSchoolLevel} schools across Tanzania.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-800">
              {filterSchoolLevel === "all" ? `📄 All ${userSchoolLevel.charAt(0).toUpperCase() + userSchoolLevel.slice(1)} Past Papers` : 
               `📄 ${userSchoolLevel.charAt(0).toUpperCase() + userSchoolLevel.slice(1)} Past Papers`}
            </h2>
            <p className="text-sm text-gray-500">
              {filterSchoolLevel === "all" ? `Showing all ${userSchoolLevel} schools` : `Showing ${userSchoolLevel} schools`}
            </p>
          </div>
                ...
                <DialogContent className="max-w-2xl bg-white/95 backdrop-blur-sm">
                  ...
                  <form onSubmit={handleUpload}>
                    <div className="grid gap-4 py-4">
                        ...
                        >
                          ...
                          <SelectContent className="bg-white">
                            {loadingSubjects ? (
                              ...
                              <SelectItem value="none" disabled>No subjects assigned to you</SelectItem>
                            ) : (
                              subjects.map((subj) => (
                      ...
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label className="text-right font-semibold">Exam Type *</Label>
                        <Select
                      ...
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label className="text-right font-semibold">School Level *</Label>
                        <Select
                          ...
                          <SelectContent className="bg-white">
                            {["primary", "secondary", "advanced"].map((level) => (
                              <SelectItem key={level} value={level}>
                                {level === "primary" ? "🏫 Primary School" : level === "secondary" ? "📚 Secondary School" : "🎓 Advanced Level"}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label className="text-right font-semibold">Class Level *</Label>
                        <Select
                        ...
                        >
                          ...
                          <SelectContent className="bg-white">
                            {CLASS_LEVELS[formData.school_level as keyof typeof CLASS_LEVELS]?.map((level) => (
                              <SelectItem key={level} value={level}>
                                {level}
                              </SelectItem>
                            ))}
                      ...
                      <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input
                          ...
                          required
                        />
                        <p className="text-xs text-gray-400 col-span-3 col-start-2">PDF, DOC, DOCX, TXT files only (Max 10MB)</p>
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <Card className="shadow-lg border-0 overflow-hidden">
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              {/* 🔥 School Level Filter - Only user's level and All Schools */}
            ...
            <div className="flex justify-end mt-4">
              <Button
                ...
                onClick={() => {
                  ...
                  setFilterSchoolLevel(userSchoolLevel) // 🔥 Reset to user's level
                  setSearchTerm("")
                  fetchPastPapers(token)
                }}
                ...
                <TableBody>
                  {filteredPapers.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center py-12">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-sm text-gray-400">Try adjusting your filters or upload a new paper!</p>
                          {canUpload && (
                            <Button 
                    ...
                    filteredPapers.map((paper) => (
                      <TableRow key={paper.id} className="hover:bg-gradient-to-r hover:from-amber-50/50 hover:to-orange-50/50 transition-all duration-200 group">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div className="h-8 w-8 rounded-full bg-gradient-to-r from-amber-500 to-orange-600 flex items-center justify-center text-white text-sm font-bold">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm text-gray-600">{paper.school_name || "Unknown School"}</span>
                          </div>
                        </TableCell>
        ...
        {/* Info Box */}
        <Card className="shadow-lg border-0 overflow-hidden bg-gradient-to-r from-amber-50 to-orange-50">
          <div className="h-1 w-full bg-gradient-to-r from-amber-500 via-orange-500 to-red-500" />
          <CardContent className="pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-medium text-gray-700">View & Download</span>
                  <p className="text-xs text-gray-500">All teachers and students can view and download past papers from all schools</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Teachers can only upload past papers for subjects they teach</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Papers are shared across all schools for collaborative learning</p>
                </div>
              </div>
            </div>
          </CardContent>
