...
const getRoleForBackend = (role: string) => {
  switch (role) {
    case "Mwalimu Mkuu":
      return "Headmaster";
...
export default function UsimamiziWaWalimuPage() {
  ...
  const handleCreateTeacher = async (e: React.FormEvent) => {
    ...
    try {
      const roleForBackend = getRoleForBackend(formData.role);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || "Imeshindwa kuongeza mwalimu");
      }
      ...
      setError(err.message || "Imeshindwa kuongeza mwalimu");
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleDeleteTeacher = async (id: number) => {
    if (
      !confirm(
        "⚠️ Je, una uhakika unataka kumfuta mwalimu huyu? Hii itafuta madarasa na masomo yote waliyopangiwa."
      )
    )
      return;
      ...
      <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6 max-w-7xl mx-auto animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="gap-1.5 sm:gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-xs sm:text-sm h-9 sm:h-10 touch-feedback w-full sm:w-auto">
                <Plus className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                <span className="hidden xs:inline">Ongeza Mwalimu</span>
            ...
            <DialogContent className="max-w-[95vw] sm:max-w-2xl bg-white rounded-2xl p-4 sm:p-6">
              <DialogHeader>
                ...
                <DialogDescription className="text-sm sm:text-base">
                  Jaza taarifa ili kuongeza mwalimu mpya.
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateTeacher}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 py-4">
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                    <Label className="text-sm font-semibold text-gray-700">Nenosiri *</Label>
                    <Input
                      type="password"
                      className="bg-white border-gray-200 focus:ring-2 focus:ring-emerald-500 rounded-xl h-10 sm:h-11 text-sm"
            ...
            <div className="flex items-start gap-2 sm:gap-3">
              <div className="flex-shrink-0 mt-0.5">
                <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-emerald-100 flex items-center justify-center">
                  <UserPlus className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-emerald-600" />
                </div>
              </div>
              <div>
                <p className="font-medium text-emerald-800 text-xs sm:text-sm">➕ Pangia Mwalimu</p>
        ...
        <div className="text-center text-[10px] sm:text-xs text-gray-400 py-3 sm:py-4 border-t border-gray-100 animate-fadeIn" style={{ animationDelay: "400ms" }}>
          ...
          <p className="mt-0.5 flex flex-wrap items-center justify-center gap-2">
            ...
            <span>•</span>
            <span>📚 {filteredCount} wamechujwa</span>
          </p>
        </div>
      </div>
