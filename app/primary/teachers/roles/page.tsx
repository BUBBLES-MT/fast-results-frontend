...
// 🔥 MAJUKUMU - KISWAHILI TU! (ZOTE ZINAENDA BACKEND KWA KISWAHILI)
const MAJUKUMU_YOTE = [
...
// 🔥 HII NDIO COMPONENT KUU - LAZIMA IWE DEFAULT EXPORT!
export default function UsimamiziWaMajukumuPage() {
  ...
  useEffect(() => {
    ...
    // 🔥 ROLES ZINAZORUHUSIWA - ZOTE KWA KISWAHILI
  ...
  const handleRoleChange = async (teacherId: number, newRole: string) => {
    ...
    setSuccess("")
    try {
      const response = await fetch(`/api/v1/primary/teachers/${teacherId}/role`, {
        method: "PUT",
        headers: {
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-sky-700 via-blue-700 to-indigo-700 p-8 text-white shadow-xl">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sky-100 max-w-2xl">
              Pangia na simamia majukumu ya walimu ili kudhibiti upatikanaji wa mfumo.
            </p>
          </div>
        </div>
                    ...
                    filteredTeachers.map((teacher, idx) => (
                      <TableRow key={teacher.id} className="hover:bg-gradient-to-r hover:from-sky-50/50 hover:to-blue-50/50 transition-all duration-200 group">
                        ...
                        <TableCell>
                          ...
                          >
                            <SelectTrigger className="w-48 bg-white border-gray-200 rounded-xl focus:ring-2 focus:ring-sky-500">
