// app/primary/academic/classes/page.tsx
...
export default function MadarasaPage() {
  ...
  const fetchSchools = async (authToken: string) => {
    try {
      ...
      const primarySchools = data.filter(
        (school: School) =>
          school.school_level === "primary" || school.school_type === "primary"
      );
      setSchools(primarySchools);
      if (primarySchools.length > 0) {
        setFormData((prev) => ({ ...prev, school_id: primarySchools[0].id }));
        setEditFormData((prev) => ({ ...prev, school_id: primarySchools[0].id }));
      }
    } catch (err) {
  ...
  const handleCreateClass = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setFormData({ name: "", school_id: schools[0]?.id || 1 });
  ...
  const handleDeleteClass = async (id: number) => {
    if (
      !confirm(
        "Je, una uhakika unataka kufuta darasa hili? Hii itafuta pia wanafunzi na alama zote katika darasa hili!"
      )
    )
      return;
    try {
      ...
      setSuccess("Darasa limefutwa kikamilifu! 🗑️");
      setTimeout(() => setSuccess(""), 3000);
      fetchClasses(token);
    } catch (err) {
    ...
    <MainLayout>
      <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6 max-w-7xl mx-auto animate-fadeIn">
        ...
        <MobileHeader
          ...
          subtitle="Simamia madarasa yote ya shule ya msingi"
              ...
              <DialogContent className="max-w-[95vw] sm:max-w-md bg-white rounded-2xl p-4 sm:p-6">
                ...
                <form onSubmit={handleCreateClass}>
                  <div className="space-y-3 sm:space-y-4 py-4">
                    ...
                    <div className="space-y-1.5 sm:space-y-2">
                      <Label className="text-sm font-semibold text-gray-700">Shule *</Label>
                      <Select
          ...
          <CardHeader className="p-4 sm:p-6 bg-white/50 backdrop-blur-sm border-b border-gray-100">
            <CardTitle className="flex items-center gap-2 text-gray-800 text-base sm:text-lg">
              ...
              <span className="text-sm font-normal text-gray-400 ml-2">
                ({totalClasses} {totalClasses === 1 ? "darasa" : "madarasa"})
              </span>
            </CardTitle>
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50/80">
                    ...
                    <TableHead className="text-xs sm:text-sm hidden sm:table-cell">Shule</TableHead>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="flex items-start gap-2 sm:gap-3">
              <div className="flex-shrink-0 mt-0.5">
                <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-sky-100 flex items-center justify-center">
                  <GraduationCap className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-sky-600" />
                </div>
              </div>
              <div>
                <p className="font-medium text-sky-800 text-xs sm:text-sm">📚 Madarasa ya Msingi</p>
                <p className="text-[10px] sm:text-xs text-sky-600/80 mt-0.5">
                  Madarasa ni nguzo kuu za shirika kwa wanafunzi, masomo na walimu
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-xl p-3 sm:p-4 animate-slideIn" style={{ animationDelay: "200ms" }}>
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div>
                <p className="font-medium text-emerald-800 text-xs sm:text-sm">👨‍🎓 Wanafunzi</p>
                <p className="text-[10px] sm:text-xs text-emerald-600/80 mt-0.5">
                  Kila darasa lina wanafunzi wanaosoma pamoja
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-100 rounded-xl p-3 sm:p-4 animate-slideIn" style={{ animationDelay: "300ms" }}>
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div>
                <p className="font-medium text-purple-800 text-xs sm:text-sm">📖 Masomo</p>
                <p className="text-[10px] sm:text-xs text-purple-600/80 mt-0.5">
                  Kila darasa lina masomo yake maalum kulingana na mtaala
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="text-center text-[10px] sm:text-xs text-gray-400 py-3 sm:py-4 border-t border-gray-100 animate-fadeIn" style={{ animationDelay: "400ms" }}>
          <p className="font-medium text-sky-600">© 2026 MASI FAST RESULTS • Usimamizi wa Madarasa</p>
          <p className="mt-0.5 flex flex-wrap items-center justify-center gap-2">
            ...
            <span>•</span>
            <span>👨‍🎓 {totalClasses * 30} wanafunzi</span>
          </p>
        </div>
      </div>
